...
if __name__ == '__main__':
    ...
    # folder_ref = '/data/kangshuai/nnunet/nnUNetFrame/DATASET/nnUNet_raw/Dataset009_Spleen/labelsTr'
    # folder_pred = '/data/kangshuai/nnunet/nnUNetFrame/DATASET/nnUNet_trained_models/Dataset009_Spleen/nnUNetTrainer__nnUNetPlans__3d_fullres/fold_4/validation'
    seg_path = r'D:\kennys\temporary\lung_nodule\airway\zoo\Connectivity-Aware-Airway-Segmentaion\sample\pred'
    gd_path = r'D:\kennys\temporary\lung_nodule\airway\zoo\Connectivity-Aware-Airway-Segmentaion\sample\mask'
    save_dir = './'
    ...
    for name in seg:
        if not name.startswith('.') and name.endswith('nii.gz'):
            ...
            specs.append(spec)
    # 存入pandas
    data = {'dice': dices, 'RVE': rves, 'Sens': senss, 'Spec': specs}
    df = pd.DataFrame(data=data, columns=['dice', 'RVE', 'Sens', 'Spec'], index=case_name)
    df.to_csv(os.path.join(save_dir, 'airway_metrics.csv'))
