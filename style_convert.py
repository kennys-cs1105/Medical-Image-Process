...
net = nn.Sequential(*[
    ...
    for i in range(max(style_layer + content_layer + 1))
])
...
def get_contents(image_shape, device):
    ...
    content_Y = extract_features(content_X, content_layer, style_layer)
    return content_X, content_Y

def get_styles(image_shape,device):
    style_X = preprocess(style_img, image_shape).to(device)
    style_Y = extract_features(style_X, content_layer, style_layer)
...
def compute_loss(X, contents_Y_hat, styles_Y_hat, contents_Y, styles_Y_gram):
    ...
    styles_l = [
        style_loss(Y_hat, gram_Y) * style_weight
...
def train(X, contents_Y, styles_Y, device, lr, num_epochs, lr_decay_epoch):
    ...
    for epoch in range(num_epochs):
        trainer.zero_grad()
        contents_Y_hat, styles_Y_hat = extract_features(X, content_layer, style_layer)
